...
describe("or", function() {
  ...
  describe("assert", function() {
    it("returns the value if value passes", function() {
      expect(or("is string or number?", isNumber, isString).assert(12)).to.eq(
        12
      );
    });
  ...
  describe("explain", function() {
    it("returns true and logs nothing if correct", function() {
      ...
      expect(
        or("is string or number?", isNumber, isString).explain(12, [])
      ).to.eq(true);
    ...
    it("returns false and logs error if spec fails", function() {
      let spy = sinon.spy(console, "log");
      expect(or("is odd or even?", even, odd).explain("hi", ["path"])).to.eq(
        false
      );
